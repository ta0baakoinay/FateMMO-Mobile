...
export function createSkillList({
	...
	Component.onAppend = function onAppend() {
		...
		// Mobile: the CSS forces the list (mini) view, but the mini + tree
		// radios share one group and the tree's #tab-1 is the one marked
		// checked in the HTML - so no mini tab is selected and the list is
		// blank. Select the first mini tab if none is active.
		const mobile = typeof window !== 'undefined' && window.ROConfig && window.ROConfig.mobileUI === true;
		if (mobile && hasTabs && !root.querySelector('.tab-switch-mini:checked')) {
			const firstMini = root.querySelector('.tab-switch-mini');
			if (firstMini) {
				firstMini.checked = true;
			}
		}
	};
