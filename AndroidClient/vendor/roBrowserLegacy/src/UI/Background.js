...
class Background {
	...
	static setImage(filename, callback) {
		...
		if (Array.isArray(filename)) {
			...
			Client.loadFile(
				...
				() => {
					// The Fate GRF ships no loading/login art -> without this the
					// map-load and login screens are a black void. Fall back to the
					// splash bundled next to index.html in the APK assets.
					const fb = (typeof window !== 'undefined' && window.ROConfig && window.ROConfig.loadingFallbackImage) || 'bg_loading.jpg';
					Object.assign(_container.style, {
	...
	static setPercent(percent) {
		...
		const width = Math.min(360, Math.floor(_canvas.width * 0.7));
		const height = 16;
		const x = Math.floor((_canvas.width - width) * 0.5);
		const y = Math.floor(_canvas.height * 0.82);

		// Clear a band a little wider/taller than the bar so the caption
		// and bar don't smear across repeated draws.
		_ctx.clearRect(x - 4, y - 26, width + 8, height + 34);

		// Draw Rectangle border
		_ctx.fillStyle = 'rgb(0,220,225)';
		_ctx.fillRect(x, y, width, height);

		// Draw Rectangle "empty"
		_ctx.fillStyle = 'rgb(30,32,40)';
		_ctx.fillRect(x + 1, y + 1, width - 2, height - 2);

		// Draw progressbar
		_ctx.fillStyle = 'rgb(120,90,190)';
		_ctx.fillRect(x + 2, y + 2, Math.floor(percent * (width - 4) * 0.01), height - 4);

		// Caption + percent
		_ctx.fillStyle = 'rgb(235,235,240)';
		_ctx.font = '13px Arial, sans-serif';
		_ctx.textBaseline = 'alphabetic';
		const label = 'Loading  ' + percent + '%';
		_ctx.fillText(label, Math.floor((_canvas.width - _ctx.measureText(label).width) * 0.5), y - 10);
	}
}
