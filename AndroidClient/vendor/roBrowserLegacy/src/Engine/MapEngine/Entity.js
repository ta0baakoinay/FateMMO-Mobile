...
/**
 * Entity floating text (script command `showscript`) - PACKET.ZC.SHOWSCRIPT (0x8b3).
 *
 * This is NOT public chat: it is a text bubble rendered above the entity's head.
 * It must only update the entity dialog bubble and must never be written to the
 * ChatBox, otherwise server scripts that loop `showscript` (e.g. NPC title /
 * floating-label helpers) flood the public chat log.
 *
 * @param {object} pkt - PACKET.ZC.SHOWSCRIPT
 */
function onEntityShowScript(pkt) {
	if (ChatRoom.isOpen) {
		return;
	}

	let msg = pkt.msg.replace(/\: \|\d{2}/, ': ');
...
export default function EntityEngine() {
	...
	Network.hookPacket(PACKET.ZC.SHOWSCRIPT, onEntityShowScript);
