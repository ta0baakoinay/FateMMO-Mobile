...
// Force the mobile control layer on when this build is packaged as an app
// (ROConfig.mobileUI === true) or when the platform reports touch support,
// instead of waiting for the very first touch event.
(function forceMobileControls() {
	try {
		var cfg = (typeof window !== 'undefined' && window.ROConfig) || {};
		var forced = cfg.mobileUI === true;
		var hasTouch =
			(typeof window !== 'undefined' && 'ontouchstart' in window) ||
			(typeof navigator !== 'undefined' && navigator.maxTouchPoints > 0);
		if ((forced || hasTouch) && cfg.mobileUI !== false) {
			touchDevice();
		}
	} catch (e) {
		/* ignore */
